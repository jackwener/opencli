...
export function extractGeneratedImages(results: unknown[], imageFields: string[]) {
  return extractGeneratedAssets(results, imageFields, 'image');
}

export function extractGeneratedVideos(results: unknown[], videoFields: string[]) {
  return extractGeneratedAssets(results, videoFields, 'video');
}

export function extractGeneratedAssets(results: unknown[], fields: string[], type: 'image' | 'video') {
  const assets: Array<{ type: 'image' | 'video'; url: string; raw: unknown }> = [];
  for (const item of results) {
    if (!item || typeof item !== 'object') continue;
    for (const field of fields) {
      const value = getByPath(item as Record<string, unknown>, field);
      if (typeof value === 'string' && value.trim()) {
        assets.push({ type, url: value, raw: item });
        break;
      }
    }
  }
