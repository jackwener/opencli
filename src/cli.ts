...
function getBrowserNetworkCachePath(scope: string = DEFAULT_BROWSER_WORKSPACE): string {
...
function loadBrowserNetworkCache(scope: string = DEFAULT_BROWSER_WORKSPACE): BrowserNetworkItem[] | null {
  try {
    const raw = fs.readFileSync(getBrowserNetworkCachePath(scope), 'utf-8');
...
function saveBrowserNetworkCache(items: BrowserNetworkItem[], scope: string = DEFAULT_BROWSER_WORKSPACE): void {
...
async function getBrowserPage(targetPage?: string): Promise<import('./types.js').IPage> {
  ...
  const page = await bridge.connect({ timeout: 30, workspace: DEFAULT_BROWSER_WORKSPACE });
...
function getBrowserNetworkCacheScope(targetPage?: string): string {
  return targetPage ? `${DEFAULT_BROWSER_WORKSPACE}:${targetPage}` : DEFAULT_BROWSER_WORKSPACE;
}
...
export function createProgram(BUILTIN_CLIS: string, USER_CLIS: string): Command {
  ...
  addBrowserTabOption(browser.command('network'))
    ...
    .action(browserAction(async (page, opts) => {
      const cacheScope = getBrowserNetworkCacheScope(page.getActivePage?.());
      if (opts.detail !== undefined) {
        const cached = loadBrowserNetworkCache(cacheScope);
      ...
      if (opts.detail !== undefined) {
        ...
        saveBrowserNetworkCache(items, cacheScope);
