...
cli({
  ...
  name: 'source/delete',
  aliases: ['source-delete'],
  description: 'Delete a source in the current NotebookLM notebook via RPC',
  ...
  columns: ['source_id', 'deleted', 'source'],
  func: async (page: IPage, kwargs) => {
    ...
    if (state.kind !== 'notebook') {
      throw new EmptyResultError(
        'opencli notebooklm source-delete',
    ...
    if (sourceId) {
      ...
      if (matches.length > 1) {
        throw new CliError(
          ...
          'Use --source-id with notebooklm source delete when duplicate source titles exist.',
        );
      }
      sourceId = matches[0]!.id;
    }

    const deleted = await deleteNotebooklmSourceViaRpc(page, sourceId);
    if (!deleted) {
      throw new CliError(
        'NOTEBOOKLM_SOURCE_ID_NOT_FOUND',
        `NotebookLM source id "${sourceId}" was not found in the current notebook.`,
