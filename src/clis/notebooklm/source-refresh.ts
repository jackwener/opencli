...
cli({
  ...
  columns: ['source_id', 'refreshed', 'source'],
  func: async (page: IPage, kwargs) => {
    ...
    if (state.kind !== 'notebook') {
      throw new EmptyResultError(
        'opencli notebooklm source-refresh',
    ...
    if (sourceId) {
      ...
      if (matches.length > 1) {
        throw new CliError(
          ...
          'Use --source-id with notebooklm source refresh when duplicate source titles exist.',
        );
      }
      sourceId = matches[0]!.id;
    }

    const refreshed = await refreshNotebooklmSourceViaRpc(page, sourceId);
    if (!refreshed) {
      throw new CliError(
        'NOTEBOOKLM_SOURCE_ID_NOT_FOUND',
        `NotebookLM source id "${sourceId}" was not found in the current notebook.`,
