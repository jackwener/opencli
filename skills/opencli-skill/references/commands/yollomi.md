...
- Args:
  - `image`(required): Image URL (upload via `opencli yollomi upload`)
...
- Args:
  - `image`(required): Input image URL (upload via `opencli yollomi upload`)
  - `prompt`(required): Editing instruction (e.g. "remove the background")
...
- Args:
  ...
  - `image`(optional): Input image URL for image-to-image (upload via `opencli yollomi upload`)
  - `output`(optional): default: './yollomi-output'; Output directory
...
- Args:
  - `person`(required): Person photo URL (upload via `opencli yollomi upload`)
